(function () {
  ...
  function QcController($state, $cookies, $rootScope, $filter, $mdSidenav, QcService) {
    var vm = this;
    vm.type = ["Potting Start", "Gradeing"];
    ...
    function createchart() {
      vm.aplus = []; vm.a = []; vm.b = []; vm.notgraded = []; vm.rework = []; vm.scrap = [];

      for (var i = 0; i < vm.data.length; i++) {
        vm.xAxisData.push(vm.data[i].date);
        vm.aplus.push((vm.data[i].aplus / vm.data[i].sum) * 100);
        vm.a.push((vm.data[i].a / vm.data[i].sum) * 100);
        vm.b.push((vm.data[i].b / vm.data[i].sum) * 100);
        vm.rework.push((vm.data[i].rework / vm.data[i].sum) * 100);
      ...
      vm.chartconfig = {
        ...
        series: [
          ...
          { name: 'Rework', data: vm.rework, stack: 'all', color: '#0066ff' },
