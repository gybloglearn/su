(function () {
  ...
  MonthController.$inject = ['$state', '$cookies', '$rootScope', '$filter', '$mdSidenav', 'ShiftreportsService', 'ModulListService', 'SmplansService', '$http'];
  function MonthController($state, $cookies, $rootScope, $filter, $mdSidenav, ShiftreportsService, ModulListService, SmplansService, $http) {
    var vm = this;
      ...
      for (var w = 1; w < 13; w++) {
        for (var sh = 0; sh < shifts.length; sh++) {
          result.push({
            month: w < 10 ? "0" + w : "" + w,
            szak: shifts[sh],
            smtarget: parseFloat(plan_deploy(w, shifts[sh], 'sm')),
            smactual: parseFloat($filter('sumField')($filter('filter')(vm.data, { machine: 'SheetMaker', month: w, szak: shifts[sh] }), 'sumgoodaeq')),
            pottingtarget: parseFloat(plan_deploy(w, shifts[sh], 'potting')),
            pottingactual: parseFloat($filter('sumField')($filter('filter')(vm.data, { machine: 'Potting', month: w, szak: shifts[sh] }), 'sumoutaeq')),
            bptarget: parseFloat(plan_deploy(w, shifts[sh], 'bp')),
            bpactual: parseFloat($filter('sumField')($filter('filter')(vm.data, { machine: 'Rework', state: 'BP', month: w, szak: shifts[sh] }), 'sumaeq')),
            mintarget: plan_deploy(w, shifts[sh], 'min'),
            minactual: parseFloat($filter('sumField')($filter('filter')(vm.data, { machine: 'MTF', type: '!ZB500S_MIN', month: w, szak: shifts[sh] }), 'gradeaeq')),
          });
        }
      }
    ...
    function plan_deploy(wk, sh, mch) {
      ...
      var d = new Date(new Date().getFullYear() + "-" + (parseInt(wk)) + "-01");
      var shs = [];
      shs["A"] = 0; shs["B"] = 0; shs["C"] = 0; shs["D"] = 0; shs["T"] = 0;
      //console.log(new Date(new Date().getFullYear() + "-" + (parseInt(wk)+1) + "-01").getDay());
