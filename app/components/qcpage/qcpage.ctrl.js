(function () {
  ...
  function QcpageController($state, $cookies, $rootScope, $filter, $mdSidenav, QcpageService) {
    var vm = this;
    vm.types = ["Potting Start", "Gradeing"];
      ...
      QcpageService.get(vm.startdatenum, vm.enddatenum, vm.acttype, vm.acttime).then(function (response) {
        ...
        for (var i = 0; i < interdata.length; i++) {
          ...
          obj = {
            ...
            rework: parseFloat($filter('sumField')($filter('filter')(vm.data, { Date: interdata[i].Date, Label: "Rework" }, true), 'Value')),
    ...
    function create_chartdata() {
      ...
      for (var i = 0; i < vm.selectdata.length; i++) {
        ...
        vm.chartData[1].data.push({ cat: vm.selectdata[i].date, y: (vm.selectdata[i].rework / vm.selectdata[i].all) * 100 });
        vm.chartData[2].data.push({ cat: vm.selectdata[i].date, y: (vm.selectdata[i].notgraded / vm.selectdata[i].all) * 100 });
        vm.chartData[3].data.push({ cat: vm.selectdata[i].date, y: (vm.selectdata[i].b / vm.selectdata[i].all) * 100 });
        vm.chartData[4].data.push({ cat: vm.selectdata[i].date, y: (vm.selectdata[i].a / vm.selectdata[i].all) * 100 });
        vm.chartData[5].data.push({ cat: vm.selectdata[i].date, y: (vm.selectdata[i].aplus / vm.selectdata[i].all) * 100 });
      }
      drowchart();
    }
